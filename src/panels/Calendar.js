...
import { format, addMonths, subMonths, getWeeksInMonth, startOfWeek, startOfMonth, endOfYesterday, isSameMonth, addDays, isSameDay, isAfter, isBefore, parse } from 'date-fns';
import ruLocale from 'date-fns/locale/ru';

import { Panel, PanelHeader, PanelHeaderButton, PanelHeaderContent, Title, Button, Group, Cell, Avatar, Snackbar } from '@vkontakte/vkui';
...
class Calendar extends Component {
        ...
        if (cloneDay === date) {
          ...
          result.push(
            <div key={user[0]} className="calendar_body_cell_item">
              <a href={"https://vk.com/id"+profile.viewer_id} title={profile.first_name+" "+profile.last_name} target="_blank" rel="noopener noreferrer">
                <Avatar src={profile.photo_100} alt={profile.first_name+" "+profile.last_name} size={24} />
              </a>
            </div>
          );
        }
  ...
  renderCells = () => {
    const { selectedUserDates, weekDay, period, blockedDays, currentDate } = this.props;
    const { currentDateCalendar } = this.state;
    const weeksNum = getWeeksInMonth(currentDateCalendar, {weekStartsOn: 1});
    let day = startOfWeek( startOfMonth(currentDateCalendar), {weekStartsOn: 1});
        ...
        <div className="row" key={weekCount}>
          {Array.apply(null, Array(7)).map((elem, dayCount) => {
            ...
            const innactive = ( 
              ...
              || isBefore(day, parse( period[0], 'yyyy-MM-dd', new Date(), {locale: ruLocale}) ) 
              || isAfter(day, parse( period[1], 'yyyy-MM-dd', new Date(), {locale: ruLocale}) ) 
            );
            ...
            return (
              <div className={`col calendar_body_cell ${isCurrentMonth&& "another"} ${dayIsIncludes&& "selected"} ${innactive&& "unselected"} `} key={dayCount} >
                <div className={`row calendar_body_cell_control ${innactive&& "innactive"}`} onClick={() => {this.onDateClick(cloneDay, innactive)}}>
                  <span className={`calendar_body_cell_icon ${(innactive)?"innactive":(dayIsIncludes)?"red":"green"}`} >
                    {(!innactive) ?
                      (dayIsIncludes) ? 
                        <Icon24UserOutgoing fill="#4986cc" /> : <Icon24UserAdded fill="#70b014" /> :
                      (dayIsIncludes) ? 
                        <Icon24User fill="#70b014" /> : <Icon24UserOutline fill="#4986cc" />
                    }
                  </span>
                  <span className="calendar_body_cell_count">{ (users.length > 0) ? users.length:" "}</span>
                  <span className={`calendar_body_cell_number ${isToday&& "today"}`}>{displayDay}</span>
                </div>
  ...
  render = () =>
    <Panel id={this.props.id}>
      <PanelHeader className={(this.props.platform[1]==='mobile_web')&& 'PanelHeader-mob-settingButton'}
        left={<PanelHeaderButton onClick={()=>{this.props.changePanel(0)}}>
          <Icon28ChevronBack />
        </PanelHeaderButton>}
        right={(this.props.rights > -1)&& <PanelHeaderContent>
          <Button before={<Icon24GearOutline style={{padding: "4px 0"}} />} 
           onClick={()=>{this.props.changePanel(2)}}>
          {!this.props.platform[0]&& "\xa0Настройки"}
          </Button>
        </PanelHeaderContent>}
      >
        <Title style={{marginLeft:0}} level={this.props.platform[1]==="mobile_web"?"3":"2"} weight="regular">Календарь посещений</Title>
      </PanelHeader>
...
const mapStateToProps = ({ group_id, group_role, admin_level, viewer_id, fetchedUser, selectedDates, sessions, selectedUserDates, accessMember, autoUpd, weekDay, period, blockedDays, currentDate, AutoUpdEnabled, activePanel, rights, platform }) =>
  ({ group_id, group_role, admin_level, viewer_id, fetchedUser, selectedDates, sessions, selectedUserDates, accessMember, autoUpd, weekDay, period, blockedDays, currentDate, AutoUpdEnabled, activePanel, rights, platform });
